...
class Ev():
    ...
    def __init__(self, iface, mac):
        ...
        #joe
        self.config["energy_transfer_mode_count"] = 1
        self.config["energy_transfer_mode"] = [0]
        #self.config["energy_transfer_mode_count"] = 2
        #self.config["energy_transfer_mode"] = [1, 4]
        #self.config["energy_transfer_mode"] = [0, 3]
    ...
    def _handleNetworkEstablished(self):
        ...
        #self.battery.setSOC(50)
        #print("201 battery get soc = ", self.battery.getSOC())
        while True:
    ...
    def _handleSessionStarted(self, data):
        ...
        print("[message] = ")
        print(message)
        print("self energy transfer mode = ", self.currentEnergyTransferMode)
    ...
    def _handleDCChargeParametersChanged(self, data):
        ...
        print("[DC Changed]battery.target_voltage = ", self.battery.target_voltage)
        print("[DC Changed]battery.target_voltage_delta = ", self.battery.target_voltage_delta)
        self.battery.in_voltage = message["evse_present_voltage"]
        print("[DC Changed]battery.in_voltage = ", self.battery.in_voltage)
        ...
        print("[DC Changed]battery.in_current = ", self.battery.in_current)
        print("[DC Changed]battery.max_current = ", self.battery.max_current)
        ...
        print("[DC Changed]battery.max_power = ", self.battery.max_power)
    ...
    def _handleChargingReady(self, data):
        ...
        print("self.currentEnergyTransferMode = ", self.currentEnergyTransferMode)
        print("battery.in_voltage = ", self.battery.in_voltage)
        print("battery.max_voltage = ", self.battery.max_voltage)
        print("battery.target_voltage = ", self.battery.target_voltage)
        print("battery.target_voltage_delta = ", self.battery.target_voltage_delta)
        # DC
        if self.currentEnergyTransferMode < 4:
            if self.battery.in_voltage < self.battery.max_voltage:
                ...
                and self.battery.in_voltage >= self.battery.target_voltage - self.battery.target_voltage_delta:
                    startCharging = True
                    print("start charging ", startCharging)
        # AC
    ...
    def loop(self):
        ...
        """
        #self.whitebeet.v2gSetConfiguration(self.config)
        #self.whitebeet.v2gSetSdpConfig()
        #self.whitebeet.v2gGetSdpConfig()
        #self.whitebeet.v2gSetDCChargingParameters(self.DCchargingParams)
        self._initialize()
